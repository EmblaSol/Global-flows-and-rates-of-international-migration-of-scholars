...
library(here)
scopus_country_flows <- read.csv(
  here::here("data_processed", "scopus_2024_V1_scholarlymigration_countryflows_enriched.csv")
)
...
As you can see people, basically just go to the US and the UK and the migration is not that high (BUT IT EXISTS). Let’s add immigration as well. 
...
base_plot <- ggplot() +
  geom_sf(data = countries, fill = NA, col = "grey60", linewidth = 0.05) +
  theme_classic(base_size = 20) +
  labs(title = "Flows from and to Norway (1998-2018)",
       fill = "") +
  theme(
    axis.line = element_blank(),
    axis.text = element_blank(),
...
Yes, you are probably completely blown away right now. Thank you so much.
